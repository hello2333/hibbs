...
CREATE TABLE `post_reply` (
  `post_reply_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `post_eval_id` int(10) unsigned DEFAULT NULL COMMENT '回复所属的评论ID（外键）',
  `post_reply_reply_id` int(10) unsigned DEFAULT NULL COMMENT '回复所属的回复ID（外键）',
  `post_reply_user_id` int(10) unsigned DEFAULT NULL COMMENT '回复人ID（外键）',
  `post_reply_text` text NOT NULL COMMENT '回复内容',
  `post_reply_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '回复时间',
  `post_reply_reply_count` int(11) DEFAULT NULL COMMENT '回复所有的回复数（只统计父亲是这个回复的回复，孙子什么的不进行嵌套计算',
  `post_reply_state` tinyint(1) DEFAULT '0' COMMENT '回复已读未读状态',
  PRIMARY KEY (`post_reply_id`),
  KEY `post_eval_id` (`post_eval_id`),
  KEY `post_reply_user_id` (`post_reply_user_id`),
  KEY `post_reply_reply_id` (`post_reply_reply_id`),
  CONSTRAINT `post_reply_ibfk_1` FOREIGN KEY (`post_eval_id`) REFERENCES `post_comment` (`post_comm_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `post_reply_ibfk_2` FOREIGN KEY (`post_reply_user_id`) REFERENCES `user` (`id`) ON DELETE SET NULL ON UPDATE SET NULL,
  CONSTRAINT `post_reply_ibfk_3` FOREIGN KEY (`post_reply_reply_id`) REFERENCES `post_reply` (`post_reply_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2016-04-05 20:35:26
